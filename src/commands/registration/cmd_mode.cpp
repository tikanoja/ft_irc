...
/*   Updated: 2023/10/12 11:23:21 by djagusch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int cmd_mode(IRCServer& server, User& user, Message& message){
	...
	std::string removals;
	bool unsettable_flag = 0;
	for (size_t i = 1; i < message.getParams().size(); i++){
		for (size_t j = 1; j < message.getParams()[j].size(); j++){
			size_t forbidden;
			if (message.getParams()[i][j] != '+' && message.getParams()[i][j] != '-')
				forbidden = message.getParams()[i].substr(j + 1).find_first_not_of(SETTABLE_MODES);
			if (forbidden != std::string::npos
					&& message.getParams()[i].c_str()[forbidden] != '+'
					&& message.getParams()[i][forbidden] != '-'){
				if (!unsettable_flag)
